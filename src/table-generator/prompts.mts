...
export function getGenerateInitialSearchQueriesPrompt(
  ...
  return `You are in charge of generating a list of search queries that will be used fill the rows in a table. These search queries will be passed to a search engine.
    ...
    The target number of rows is ${minRequiredRows}, so make sure the queries you write are broad enough to give you back a sufficiently LONG list of results.
    ...
    You should focus on getting a good list of unique primary key values. Don't output more than 4 queries. Only write queries if they are meaningfully different from each other. Don't rewrite the same query multiple ways.`;
}
...
export function getGenerateAdditionalSearchQueriesPrompt(
  ...
  return `You are in charge of generating a list of search queries that will be used fill the rows in a table. These queries will be passed to a search engine.
    ...
    So far, we have only found ${Object.keys(rows).length} rows, we need to find ${minRequiredRows} rows.
    So make sure the queries you write are broad enough to give you back a sufficiently LONG list of results.

    You should generate a list of search queries that will be used to fill the rows in the table. You should not capture all of the columns right now, focus on searching for the primary key column. 
    Each of these queries should be broad, and should aim to give you back a LONG list of results. Ask for lists, not about specific records. You are building the list right now, you will search for the list properties later.
    You should focus on getting a good list of unique primary key values. Don't output more than 4 queries.
